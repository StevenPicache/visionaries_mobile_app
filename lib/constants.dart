


const double kSmallMargin = 10;
const double kLargeMargin = 20;
const double kListViewWidth = 400;
const double kListViewHeight = 400;
const double kScreenHeight = 600;
const double kScreenWidth = 400;


// API URL IP_ADDRESS
const String EMULATOR_API_URL_ANDROID = 'http://10.0.2.2:';
const String EMULATOR_API_URL_IOS = 'http://127.0.0.1:';
const String PORT_NUMBER = '5000';




// API URL ADDRESS
const String API_SERVICES_URL_WORKORDERS = '/workorders';
const String API_SERVICES_URL_AUTH = '/auth';
const String API_SERVICES_URL_USERS = '/users';
const String API_SERVICES_URL_CONTRACTS = '/contracts';
const String API_SERVICES_URL_INVENTORY = '/inventory';
const String API_SERVICES_URL_TASK_FINISHED = '/workorders';
const String API_SERVICES_URL_ITEM_SEARCH = '/itemSearch';




// SHARED PREFERENCE TOKEN CONSTANTS
const USER_TOKEN_KEY = "token";
const USER_ID_KEY = "userid";